...
import { interviewer } from "@/constants/index"; // adjust path as needed
...
const Agent: React.FC<AgentProps> = ({
  ...
  const handleGenerateFeedback = async(mesages:
    SavedMessage[]) =>{
...
const handleCall = async () => {
  try {
    ...
    if (!workflowId) throw new Error("❌ No workflow ID found");
    ...
    console.log("🎙️ Starting interviewer with questions:", formattedQuestions);

    // ✅ Build variableValues safely
...
await vapi.start(interviewer, {
  variableValues: {
    username: userName,
    userid: userId,
    role,           // ✅ make sure this variable is filled
    level,
    techstack,
    questions: formattedQuestions,
  },
...
// const Agent: React.FC<AgentProps> = ({ userName, userId,type,interviewId,questions, role,level,techstack,amount}) => {
//   const router = useRouter();
...
// //       router.push(`/interview/${id}/feedback`);
// //     } else {
// //       console.error("❌ Error saving feedback — invalid response", { success, id });
// //       router.push("/");
// //     }
// //   } catch (error) {
